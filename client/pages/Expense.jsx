            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 overflow-hidden">
                <div className="bg-gradient-to-r from-rose-600 to-rose-500 px-6 py-4 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <h2 className="text-white font-bold text-lg">{isEdit ? 'Edit Expense Entry' : 'Add Expense Entry'}</h2>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="col-span-2">
                            ...
                            <input
                                ...
                                className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-rose-500/30"
                            />
                        </div>
                        <div>
                        ...
                        <div className="col-span-2">
                            ...
                            <textarea
                                ...
                                className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-rose-500/30"
                            />
                        </div>
                    </div>
                </div>
                ...
                <div className="px-6 pb-6 flex justify-end gap-3">
                    ...
                    <button
                        onClick={onSave}
                        disabled={isSaving}
                        className="flex items-center gap-2 bg-rose-600 hover:bg-rose-700 text-white font-bold rounded-lg px-5 py-2 text-sm transition disabled:opacity-50"
                    >
...
export default function Expense() {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            const [listRes, sumRes] = await Promise.all([
                api(`/api/income-expense?type=expense&month_year=${monthYear}`),
    ...
    return (
        <DashboardLayout activeRoute="expense">
            <div className="flex flex-col h-full gap-4 p-4 bg-gray-50 min-h-screen">
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-xl font-black text-gray-900">Expense</h1>
                            <p className="text-xs text-gray-400">Track all expense entries per month</p>
                        </div>
                    </div>
                    <button
                        onClick={() => dispatch({ type: 'OPEN_ADD' })}
                        className="flex items-center gap-2 bg-rose-600 hover:bg-rose-700 text-white font-bold rounded-xl px-4 py-2 text-sm transition shadow"
                    >
                ...
                <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 flex flex-wrap items-center gap-4">
                    ...
                    <select
                        ...
                        className="border border-gray-200 rounded-lg px-3 py-1.5 text-sm font-bold outline-none focus:ring-2 focus:ring-rose-500/20"
                    >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div className="bg-gradient-to-br from-rose-500 to-rose-600 rounded-2xl p-5 text-white shadow-lg">
                        <p className="text-rose-100 text-xs font-bold uppercase tracking-wider">Total Expense</p>
                        <p className="text-3xl font-black mt-1">{fmtCurrency(totalExpense)}</p>
                        <p className="text-rose-100 text-xs mt-1">{monthName(month)} {year}</p>
                    </div>
                        ...
                        <table className="w-full text-sm">
                                    ...
                                    <tr>
                                        <td colSpan={7} className="py-20 text-center">
                                            <div className="flex flex-col items-center gap-2">
                                                ...
                                                <p className="text-gray-400">Loading expense entries…</p>
                                            </div>
                                        </td>
                                    </tr>
                                ) : entries.length === 0 ? (
                                    <tr>
                                        <td colSpan={7} className="py-20 text-center">
                                            <div className="flex flex-col items-center gap-3">
                                                ...
                                                <div>
                                                    <p className="font-bold text-gray-700">No expense entries for {monthName(month)} {year}</p>
                                                    <p className="text-xs text-gray-400 mt-1">Click &quot;Add Expense&quot; to create your first entry</p>
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
                                    ...
                                    entries.map((entry, i) => (
                                        <tr key={entry.id} className="hover:bg-rose-50/30 transition-colors">
                                            ...
                                            <td className="px-4 py-3">
                                                <span className="inline-flex items-center gap-1 bg-rose-50 text-rose-700 text-xs font-bold px-2 py-1 rounded-md">
                                            ...
                                            <td className="px-4 py-3 text-right font-black text-rose-600">{fmtCurrency(entry.amount)}</td>
                                            <td className="px-4 py-3">
                            ...
                            {entries.length > 0 && (
                                <tfoot className="bg-rose-50 border-t-2 border-rose-100">
                                    <tr>
                                        <td colSpan={5} className="px-4 py-3 text-right font-black text-xs uppercase tracking-widest text-rose-700">Total Expense</td>
                                        <td className="px-4 py-3 text-right font-black text-rose-700 text-base">{fmtCurrency(totalExpense)}</td>
                                        <td />
                                    </tr>
                                </tfoot>
                            )}
