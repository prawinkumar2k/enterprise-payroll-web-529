                ...
                <div className="flex-1 overflow-auto p-6 no-print">
                    <div className="bg-white rounded-xl shadow-sm border p-6 min-w-[800px]">
                        <div className="flex justify-between items-center mb-6">
                            <h3 className="text-lg font-bold text-gray-800 uppercase tracking-tight">Payment Abstract Preview</h3>
                        ...
                        {isLoading ? (
                            <div className="flex flex-col items-center justify-center h-64 gap-4">
                                ...
                                <span className="text-sm font-bold text-gray-400">Compiling Distribution Summary...</span>
                            </div>
                        ) : reportData.length === 0 ? (
                                ...
                                <table className="w-full text-[12px]">
                                    <thead className="bg-gray-50 border-b">
                                        <tr className="divide-x">
                                            ...
                                            <th className="px-3 py-3 text-right font-bold">NET AS PER BILL</th>
                                            <th className="px-3 py-3 text-right">BANK TRANSFER</th>
                                    ...
                                    <tbody className="divide-y">
                                        {reportData.map((row, idx) => (
                                            <tr key={idx} className="divide-x hover:bg-gray-50/50">
                                                ...
                                                <td className="px-3 py-2 text-right font-bold">{fmt(row.NET)}</td>
                                                <td className="px-3 py-2 text-right text-green-600 font-bold">{fmt(row.ByBank)}</td>
                                                <td className="px-3 py-2 text-right text-amber-600">{fmt(row.ByCash)}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
