import React, { useState, useEffect, useMemo } from "react";
import DashboardLayout from "../components/DashboardLayout";
import {
    Home,
    Loader2,
    AlertCircle
} from "lucide-react";
import { useNavigate } from "react-router-dom";
import { toast } from "sonner";
import PayBillToolbar from "../components/PayBillToolbar";
import { useLatestMonthYear } from "../hooks/useLatestMonthYear";
import { REPORT_CONFIGS, p, fmt } from "../utils/printEngine";

export default function Abstract1() {
    const navigate = useNavigate();
    const { latest, availableYears } = useLatestMonthYear();

    const fallbackMonth = String(new Date().getMonth() + 1).padStart(2, "0");
    const fallbackYear = String(new Date().getFullYear());

    const [filters, setFilters] = useState({
        category: "ALL",
        month: null,
        year: null,
        withSignature: true,
        paySlip: false,
        bonusPayBill: false
    });

    // Derive active period: use latest payroll month until user explicitly changes
    const activeMonth = filters.month ?? latest?.month ?? fallbackMonth;
    const activeYear  = filters.year  ?? latest?.year  ?? fallbackYear;

    const [reportData, setReportData] = useState([]);
    const [isLoading, setIsLoading] = useState(false);
    const _rowsPerPage = REPORT_CONFIGS.ABSTRACT_1.rowsPerPage;

    const fetchReport = async () => {
        setIsLoading(true);
        const monthYear = `${activeMonth}-${activeYear}`;
        try {
            const response = await fetch(`/api/reports/abstract-1?monthYear=${monthYear}&category=${filters.category}&bonus=${filters.bonusPayBill}`, {
                headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
            });
            const data = await response.json();
            if (data.success) {
                setReportData(data.data);
            } else {
                toast.error(data.message);
                setReportData([]);
            }
        } catch {
            toast.error("Failed to fetch Abstract 1 data");
        } finally {
            setIsLoading(false);
        }
    };

    useEffect(() => {
        fetchReport();
    // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [activeMonth, activeYear, filters.category, filters.bonusPayBill]);

    const handlePrint = () => {
        navigate('/print-report', {
            state: {
                data: reportData,
                reportType: 'ABSTRACT_1',
                filters: { ...filters, month: activeMonth, year: activeYear },
                orientation: 'landscape'
            }
        });
    };
    const handleClear = () => {
        setFilters({
            ...filters,
            category: "ALL",
            month: null,
            year: null,
            bonusPayBill: false
        });
    };

    const totals = useMemo(() => {
        return reportData.reduce((acc, r) => {
            acc.gross += p(r.GrossPay);
            acc.epf += p(r.EPF);
            acc.esi += p(r.ESI);
            acc.it += p(r.IT);
            acc.pt += p(r.PT);
            acc.net += p(r.NetPay);
            return acc;
        }, { gross: 0, epf: 0, esi: 0, it: 0, pt: 0, net: 0 });
    }, [reportData]);



    return (
        <DashboardLayout activeRoute="reports">
            <div className="flex flex-col h-full bg-gray-100/50">
                {/* TOOLBAR */}
                <div className="flex items-center gap-4 bg-white border-b px-6 py-2 no-print shadow-sm z-10">
                    <button
                        onClick={() => navigate('/dashboard')}
                        className="flex items-center gap-2 px-3 py-1.5 text-xs font-bold text-gray-600 hover:text-primary transition-colors border rounded-md"
                    >
                        <Home className="w-4 h-4" />
                        BACK
                    </button>
                    <div className="h-4 w-px bg-gray-300 mx-2" />
                    <PayBillToolbar
                        filters={{ ...filters, month: activeMonth, year: activeYear }}
                        setFilters={setFilters}
                        handleClear={handleClear}
                        handlePrint={handlePrint}
                        hasData={reportData.length > 0}
                        isAbstract={true}
                        availableYears={availableYears}
                    />
                </div>

                {/* SCREEN ONLY VIEW */}
                <div className="flex-1 overflow-auto p-6 no-print">
                    <div className="bg-white rounded-xl shadow-sm border p-6 min-w-[800px]">
                        <div className="flex justify-between items-center mb-6">
                            <h3 className="text-lg font-bold text-gray-800 uppercase tracking-tight">Statutory Abstract Preview</h3>
                            <div className="flex gap-4 items-center">
                                <div className="text-sm text-gray-500">Gross: <span className="font-bold text-gray-900">{fmt(totals.gross)}</span></div>
                                <div className="text-sm text-gray-500">Net: <span className="font-bold text-green-600">{fmt(totals.net)}</span></div>
                            </div>
                        </div>

                        {isLoading ? (
                            <div className="flex flex-col items-center justify-center h-64 gap-4">
                                <Loader2 className="w-10 h-10 animate-spin text-primary" />
                                <span className="text-sm font-bold text-gray-400">Compiling Abstract...</span>
                            </div>
                        ) : reportData.length === 0 ? (
                            <div className="flex flex-col items-center justify-center h-64 gap-4 opacity-50">
                                <AlertCircle className="w-16 h-16 text-gray-200" />
                                <p className="text-gray-900 font-bold uppercase">No Records Found</p>
                            </div>
                        ) : (
                            <div className="overflow-x-auto border rounded-lg">
                                <table className="w-full text-[12px]">
                                    <thead className="bg-gray-50 border-b">
                                        <tr className="divide-x">
                                            <th className="px-3 py-3 text-left">S.N</th>
                                            <th className="px-3 py-3 text-left">CATEGORY / GROUP</th>
                                            <th className="px-3 py-3 text-right">GROSS</th>
                                            <th className="px-3 py-3 text-right">EPF</th>
                                            <th className="px-3 py-3 text-right">ESI</th>
                                            <th className="px-3 py-3 text-right">IT</th>
                                            <th className="px-3 py-3 text-right font-bold">NET PAYABLE</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y">
                                        {reportData.map((row, idx) => (
                                            <tr key={row.Category || String(row.DGroup) || idx} className="divide-x hover:bg-gray-50/50">
                                                <td className="px-3 py-2">{idx + 1}</td>
                                                <td className="px-3 py-2 font-bold text-gray-700">{row.Category || `Group ${row.DGroup}`}</td>
                                                <td className="px-3 py-2 text-right">{fmt(row.GrossPay)}</td>
                                                <td className="px-3 py-2 text-right">{fmt(row.EPF)}</td>
                                                <td className="px-3 py-2 text-right">{fmt(row.ESI)}</td>
                                                <td className="px-3 py-2 text-right">{fmt(row.IT)}</td>
                                                <td className="px-3 py-2 text-right font-bold text-indigo-600">{fmt(row.NetPay)}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                            </div>
                        )}
                    </div>
                </div>
            </div>
        </DashboardLayout>
    );
}
